...
def test_network(cake, network, dataset):
    ...
    output_predictions = cake.run_prediction(None, dataset.test_dataset)
...
def return_predictions(predictions, index, rand=False):
    preds = np.ndarray(shape=(5, 6), dtype=DataType.ndtype)
    max_i = 0
    max_prob = 0
    ...
    for i in range(5): 
        ...
        preds[i,0] = j
        preds[i,5] = -1
        #first 11 entries of the index are probabilities of number being 0, 1, 2,... upto 11 in length
        #we have max of only 5 but 11 was the number of classes for each digit so we kept it this way
        #even for number length 
        prob = predictions[index, j]
        for k in range(5):
            if k <= i:
                start_index = (k+1)*(num_labels+1)
                end_index = start_index+num_labels
                prob = prob*max(predictions[index,start_index:end_index])
                preds[i,k+1] = np.argmax(predictions[index,start_index:end_index])
            else:
                no_digit_index = (k+1)*(num_labels+1)+num_labels
                prob = prob*(predictions[index,no_digit_index])
                preds[i,k+1] = -1
        if prob > max_prob:
            max_prob = prob
            max_i = i
